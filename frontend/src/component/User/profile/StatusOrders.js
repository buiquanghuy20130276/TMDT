...
const StatusOrders = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            await axios.get(`http://localhost:8080/api/v1/order/${activeTab}/orders`, {params: {email: `${user.email}`}}).then(res => {
                setOrders(res.data);
    ...
    const handleCancelOrder = (id) => {
        axios.put(`http://localhost:8080/api/v1/order/cancel-order/${id}`)
            .then(res => {
                    setRefresh(pre => !pre)
    ...
    const handleReturnOrder = (id) => {
        axios.put(`http://localhost:8080/api/v1/order/return-order/${id}`)
            .then(res => {
                    setRefresh(pre => !pre)
