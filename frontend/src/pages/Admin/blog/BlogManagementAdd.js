...
export default function BlogManagementAdd(){
    ...
    const handleImageChange = async (e) => {
        const file = e.target.files[0]; // Lấy file ảnh đầu tiên từ sự kiện change
        ...
        try {
            ...
            setBlog((prevBlog) => ({
                ...prevBlog,
                img: response.data.imageUrl // Lưu đường dẫn ảnh trên server vào trường img trong state blog
            }));
        } catch (error) {
    ...
    const createBlog=async(event)=>{
        ...
        if (blog.title !== '' && blog.description !== '' && blog.author !== '' && blog.content !== '' && blog.category !== '' && blog.img !== '' ) {
            ...
            console.log(data)
            await axios.post("http://localhost:8080/api/v1/blogs/add", data
            ).then(res => {
