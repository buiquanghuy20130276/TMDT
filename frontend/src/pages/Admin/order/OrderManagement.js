...
const OrderManagement = () => {
    ...
    useEffect(() => {
        const getOrders=async ()=>{
            await axios.get("http://localhost:8080/api/v1/order").then(res => {
                setOrders(res.data)
    ...
    const deleteMultipleOrders = async () => {
        try {
            await axios.delete('http://localhost:8080/api/v1/order-management/order/delete/orders', {data: selectedOrders,headers: {
    ...
    const confirmMuiltipleOrder = async () => {
        try {
              await axios.put('http://localhost:8080/api/v1/order-management/order/confirm/orders', selectedOrders,{headers: {
                      Authorization: `Bearer ${accessToken}`,
                  }});
            // Cập nhật lại danh sách đơn hàng sau khi xóa
            setRefresh(prev => !prev);
