...
const CommentManagement = () => {
    ...
    const [isParentToDelete, setIsParentToDelete] = useState(false);
        ...
        async function getAllComments() {
            try {
                const response = await axios.get(
                    'http://localhost:8080/api/v1/management/comment/all',
    ...
    const deleteCommentChild = async (commentId) => {
        let confirm = window.confirm("Bạn chắc chắn muốn xóa  comment này");
        try {
            if (confirm) {
                await axios.get(`http://localhost:8080/api/v1/delete/comments/${commentId}`);
                getAllComments();
