...
const ReviewManagement = () => {
    ...
    const getALlReview = async () => {
        try {
            let req = await axios.get("http://localhost:8080/api/v1/management/review", {
    ...
    //         try {
    //        let res=     await  axios.delete(`http://localhost:8080/api/v1/review/delete?id=${id}`)
    //             console.log(res.data)
    //             const updatedData = review.filter((item) => item.id !== id);
    ...
    const handleRemove = async () => {
        if (selectedReviews.length === 0) {
            toast.error("Vui lòng chọn ít nhất một đánh giá để xóa.");
            return;
        }
    ...
    const handleConfirmDelete = async () => {
        try {
            ...
            const res = await axios.delete("http://localhost:8080/api/v1/review/deletes", {
