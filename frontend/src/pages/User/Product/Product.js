...
// Vị trí trang đang đứng
    ...
    useEffect(() => {
        if (rowData && rowData.title) {
            axios.get(`http://localhost:8080/api/v1/search?page=${currentPage}&size=${productsPerPage}&keyword=${rowData.title}`)
                .then(response => {
            ...
            axios.get(`http://localhost:8080/api/v1/products?page=${currentPage}&size=${productsPerPage}&category=${type}&sort=${sortBy}`)
                .then(response => {
                    setProducts(response.data.content);
                    setTotalPages(response.data.totalPages);
                })
