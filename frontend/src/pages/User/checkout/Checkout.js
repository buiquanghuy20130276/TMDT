...
const Checkout = () => {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                const res = await axios.get("http://localhost:8080/api/v1/user/get", {params: {email: user.email}})
                setAccount(res.data)
    ...
    const handlePayment = async (e) => {
        ...
        if (paymentMethod === "vnpay") {
            try {
                ...
                const response = await axios.post("http://localhost:8080/api/v1/payment/create_payment", data);
            ...
            await axios.post("http://localhost:8080/api/v1/order/create", dataOrder).then(r => {
                dispatch(clearCart());
